...
### Tier A — Core (v1). Free, mostly already in Bybit MCP.

| Signal group | Concrete signals | Source | Cost / Key | Value | Complexity |
|---|---|---|---|---|---|
| **Trend** | EMA/SMA cross (e.g. 20/50/200), price vs MA, higher-highs/lows | Bybit MCP klines → compute in code | Free, no key | High | Low |
| **Momentum** | RSI, MACD, rate-of-change | Bybit MCP klines → compute | Free, no key | High | Low |
| **Volatility** | ATR, Bollinger width, realized vol | Bybit MCP klines → compute | Free, no key | High | Low |
| **Volume / liquidity** | Volume z-score, volume trend, order-book imbalance/spread | Bybit MCP klines + orderbook | Free, no key | Med-High | Low |
| **Derivatives positioning** | **Funding rate**, **open interest** (Δ), **long/short ratio** | Bybit MCP (market tools) | Free | High | Low |
| **Multi-timeframe** | Align signals across e.g. 15m / 1h / 4h | Bybit MCP klines (per TF) | Free | High | Low-Med |
...
Trade decisions for the configured symbol(s) on testnet are formed from a **structured feature snapshot** containing:
...
All of (1)–(6) come from the **Bybit MCP** (klines / tickers / orderbook / funding / OI). Item (7) comes from **two free keyless calls**. That's the whole v1 data surface — deliberately small.
...
```
Bybit MCP (klines/orderbook/funding/OI) ─┐
...
|---|---|---|---|---|
| **Bybit MCP** (`bybit-official-trading-server`) | MCP (stdio) | Testnet key (market data needs none) | OHLCV, orderbook, tickers, funding, OI, L/S ratio, **order execution** | v1 |
...
|---|---|---|
| **S1 — Core TA** | Tier A price/trend/momentum/volatility/volume from Bybit MCP | Highest value, lowest complexity, no extra deps |
...
S1–S3 are all v1 and require **no paid services and (mostly) no API keys** beyond the Bybit testnet credential.

---

## 7. Complexity guardrails (so this stays "high value, low complexity")

- **One source of truth per signal.** Don't pull price from three places; Bybit MCP is canonical for traded instruments.
...
### References
- Bybit MCP server — [github.com/bybit-exchange/trading-mcp](https://github.com/bybit-exchange/trading-mcp)
